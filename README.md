...
2. **`scripts/market_report_vision.py` (爬蟲引擎)**
   - 負責動態去網路上獲取卡片的即時報價。
...
3. **`scripts/market_monitor.py` (實時監控大腦)**
   - 這是你需要讓 AI Agent **全天候 24 小時在背景運行** 的主程式。

---

## 🤖 `scripts/market_monitor.py` 的核心：全實時監控 (Full Real-Time)
...
### 1. 系統環境準備
請確保這個 Repository (專案資料夾) 包含了以下核心內容：
- `scripts/` (包含 `market_monitor.py` 與 `market_report_vision.py`)
- `renaiss_full_db.sqlite` (中央資料庫)
...
### 3. 未來擴充：報表生成
目前在 `scripts/market_monitor.py` 啟動時，會自動跑一次 `generate_report()` 函數，印出近期市場上漲幅最大的榜單。
...
除了市場行情監控，系統還支援 **Twitter/X 帳號監控**，自動抓取 Pokemania 相關帳號的最新推文，透過 MiniMax AI 整理成繁體中文摘要後發送到 Discord。

## 📂 Twitter 監控架構

| 組件 | 功能 |
|------|------|
| `scripts/twitter_monitor.py` | 主監控程式（30分鐘輪詢） |
| Jina AI | 繞過 Twitter 反爬機制抓取內容 |
| MiniMax API | AI 生成推文中日文內容的繁體中文摘要 |
| Discord Webhook | 發送格式化通知到指定頻道 |
...
```bash
# Discord Webhook（Twitter 通知專用頻道）
DISCORD_WEBHOOK_URL_TWITTER="https://discord.com/api/webhooks/xxx/yyy"

# 要監控的 Twitter 帳號（可多個，用逗號分隔）
TWITTER_ACCOUNTS="pokegetinfomain"

# 檢查頻率（分鐘）
TWITTER_CHECK_INTERVAL=30

# MiniMax API Key（用於 AI 摘要）
MINIMAX_API_KEY="your_key_here"
```

## 📋 功能特點

- ✅ **Jina AI 抓取**：繞過 Twitter 反爬，穩定抓取公開內容
- ✅ **ID 去重**：自動追蹤已讀過的推文，只通知新內容
- ✅ **AI 摘要**：日文推文自動翻譯整理成重點
- ✅ **多帳號支援**：可同時監控多個 Twitter 帳號
