...
# 將 openclaw 目錄加入路徑，讓它可以載入內部的 market_report_vision
sys.path.append(os.path.join(os.path.dirname(__file__), "..", "openclaw"))
    ...
    try:
        if mode == "json":
            ...
            if res and len(res) >= 1:
                ...
                return card_info
            return {"error": "分辨失敗，未找到卡片資訊"}

        elif mode == "full":
            # 模式二：完整市場行情分析報告
            print(f"📊 [OpenClaw] 模式: FULL 報告 | 處理圖片: {os.path.basename(image_path)}")
            # REPORT_ONLY 設為 True 避免中間過多 print，只拿最後結果
